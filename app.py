...
# ============================================================
    ...
    with tab_content:
        ...
        # ------------------------------------------------------------
        # STEP 1: Upload the real marketplace template BEFORE generation.
        # The uploaded template stays available through Streamlit reruns.
        # ------------------------------------------------------------
        st.markdown("---")
        st.subheader("📄 Step 1 — Upload Marketplace Template")
        st.caption("Pehle Amazon / Flipkart / Meesho ka current vertical template upload karein. App isi template ke exact fields/column order ko use karega.")
        template_file = st.file_uploader(
            "Upload the actual marketplace vertical template",
            type=["xlsx", "xlsm", "csv"],
            key="dynamic_market_template",
            help="Amazon Seller Central / Flipkart Seller Hub / Meesho Supplier Panel se current vertical template upload karein."
        )
        if template_file:
            try:
                _preview_tdf, _preview_meta = _read_template_bytes(template_file)
                _preview_req = _template_required_flags(template_file)
                st.success(
                    f"✅ Template ready: {template_file.name} | "
                    f"{len(_preview_tdf.columns)} fields detected | {marketplace}"
                )
                st.caption(
                    f"Source: {_preview_meta.get('kind')}" +
                    (f" | Sheet: {_preview_meta.get('sheet')}" if _preview_meta.get('sheet') else "") +
                    (f" | Required fields detected: {sum(1 for v in _preview_req.values() if v)}" if _preview_req else "")
                )
            except Exception as e:
                st.error(f"Template preview error: {e}")
                template_file = None

        st.markdown("### 🚀 Step 2 — Generate Visibility Variants + Prefill Template")
        if not template_file:
            st.info("💡 Template upload optional hai for content-only generation, lekin **Final marketplace upload file** banane ke liye template upload karna required hoga.")
            ...
            if not base or not facts:
                ...
                try:
                    ...
                    if not data or not isinstance(data.get("variants"), list):
                        ...
                        # Persist variants so template/manual-field widgets survive Streamlit reruns.
                        st.session_state["multi_generated_variants"] = variants
                        st.session_state["multi_generated_marketplace"] = marketplace
                        st.session_state["multi_generated_base"] = base
                        st.session_state["multi_generated_facts"] = facts
                        st.session_state["multi_generated_category"] = category
                        st.session_state["multi_generated_product_type"] = product_type
                        st.success("✅ Variants generated. Ab template fields neeche continue karein.")

                except Exception as e:
                    st.error(f"Variant generation error: {e}")

        # ------------------------------------------------------------
        # STEP 3: Results + dynamic template builder.
        # Stored in session_state so data_editor / input changes do not
        # make the generated variants disappear on rerun.
        # ------------------------------------------------------------
        stored_variants = st.session_state.get("multi_generated_variants")
        stored_marketplace = st.session_state.get("multi_generated_marketplace", marketplace)
        stored_base = st.session_state.get("multi_generated_base", base)
        stored_facts = st.session_state.get("multi_generated_facts", facts)
        stored_category = st.session_state.get("multi_generated_category", category)
        stored_product_type = st.session_state.get("multi_generated_product_type", product_type)

        if stored_variants:
            variants = stored_variants
            qdf = variant_quality(variants, stored_marketplace)
            ...
            for v in variants:
                with st.expander(f"Variant {v.get('variant_no')} — {v.get('angle','')}"):
                    ...
                    if stored_marketplace == "Amazon India":
                        st.markdown("**Item Highlights**")
            ...
            for v in variants:
                export_rows.append({
                    "marketplace": stored_marketplace,
                    "base_product": stored_base,
            ...
            # --------------------------------------------------------
            st.markdown("---")
            st.subheader("📤 Step 3 — Direct Upload Template Builder")
            st.caption("Uploaded template ke exact columns/order ko preserve karke product facts, generated content aur seller details map kiye jayenge.")

            if not template_file:
                st.warning("⚠️ Final marketplace upload file ke liye Step 1 me template upload karein.")
            else:
                try:
                    ...
                    dyn_df, blank_fields, missing_required = build_dynamic_upload(
                        tdf, variants, stored_base, stored_facts, stored_marketplace,
                        stored_category, stored_product_type, md, req_flags
                    )

                    if blank_fields:
                        st.markdown("### 🟡 Remaining Manual Fields")
                        st.caption("Field names template se automatically aaye hain. Sirf Value column me data enter karein; field name change nahi karna hai.")
